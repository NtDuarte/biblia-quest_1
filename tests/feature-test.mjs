...
const checks = [
  ...
  ['estudo ilimitado no dia', html.includes('Você pode fazer quantos estudos quiser no mesmo dia')],
  ['botão próxima fase', html.includes('id="next-lesson"') && html.includes('continuePath')],
  ['sugestão de versículo pela IA', html.includes('id="suggest-verse"') && html.includes('suggestVerse')],
  ['API modo suggest', api.includes("mode === 'suggest'") && api.includes('Sugira a próxima fase de estudo bíblico')],
  ['perguntas bíblicas, não institucionais', html.includes('biblicalQuestions') && !html.includes('Apenas ganhar XP') && !html.includes('Qual é o objetivo principal de um estudo bíblico no Trilha VIVA')],
