...
function localFallback({ mode, verse, topic, track }) {
  if (mode === 'suggest') {
    const suggestions = [
      {icon:'🌿', title:'Crescimento constante', topic:'Maturidade espiritual', verse:'2 Pedro 1:3-8', goal:'Avançar um passo na fé com diligência.'},
      {icon:'🕊️', title:'Paz que guarda', topic:'Ansiedade e oração', verse:'Filipenses 4:4-9', goal:'Trocar ansiedade por oração e confiança.'},
      {icon:'🔥', title:'Palavra no coração', topic:'Santificação diária', verse:'Salmos 119:9-16', goal:'Guardar a Palavra para viver melhor.'},
      {icon:'🤝', title:'Vida em comunidade', topic:'Unidade cristã', verse:'Efésios 4:1-6', goal:'Praticar humildade, mansidão e paciência.'}
    ];
    const picked = suggestions[Math.floor(Math.random() * suggestions.length)];
    return { ok: true, mode, local: true, warning: 'Sugestão local gerada.', content: '', parsed: picked };
  }
  if (mode === 'doubt') {
    return {
      ok: true,
...
Pergunta de reflexão: qual pequena obediência você pode praticar hoje?`,
  ...
  return {
    ...
    parsed: {
      ...
      xp: 20,
      quiz: {
        question: 'Na parábola do bom samaritano, qual atitude Jesus ensina como exemplo de amor ao próximo?',
        options: ['Indiferença diante da dor', 'Compaixão prática e misericordiosa', 'Busca por reconhecimento público', 'Discussão sem obediência'],
        answerIndex: 1,
        explanation: 'Jesus ensina que o amor ao próximo aparece em misericórdia concreta, não apenas em palavras.'
      }
    }
  };
}
...
function buildPrompt({ mode, verse, topic, question, level, track, memory }) {
  ...
  if (mode === 'suggest') {
    return {
      system: persona,
      user: `${base}\n\nSugira a próxima fase de estudo bíblico para o aluno, escolhendo um versículo bíblico diferente e coerente com o crescimento espiritual. Responda em JSON válido, sem markdown, exatamente no formato: {"icon":"✨","title":"","topic":"","verse":"Livro capítulo:verso-verso","goal":""}. Não invente referência bíblica inexistente.`
    };
  }
...
export default async function handler(req, res) {
  ...
  if (!['doubt','study','quiz','suggest'].includes(mode)) return res.status(400).json({ ok: false, error: 'Modo inválido.' });
  if (!['doubt','suggest'].includes(mode) && !verse && !topic) return res.status(400).json({ ok: false, error: 'Informe um texto bíblico ou tema.' });
  ...
  try {
    ...
    const response = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({
        ...
        temperature: 0.7,
        max_output_tokens: mode === 'doubt' ? 650 : mode === 'suggest' ? 260 : 1100
      })
    });
